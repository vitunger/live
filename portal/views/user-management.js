...
// ═══ OFFICE-RÄUME ADMIN ═══
var _officeAdminDragMode = false;
var _officeDraggingDesk = null;

window.officeToggleDragMode = function() {
    _officeAdminDragMode = document.getElementById('officeAdminDragMode').checked;
    var hint = document.getElementById('officeAdminDragHint');
    if(hint) hint.textContent = _officeAdminDragMode ? '⚡ Drag-Modus aktiv – Ziehe die Punkte an die richtige Position. Änderungen werden sofort gespeichert.' : 'Aktiviere "Plätze verschieben" um Dots per Drag & Drop auf dem Grundriss zu positionieren.';
    officeRenderAdminDots();
};

var _officeRoomColors = ['#EF7D00','#3b82f6','#22c55e','#a855f7','#ef4444','#eab308','#06b6d4','#ec4899','#64748b','#84cc16','#f97316','#8b5cf6'];

window.officeRenderAdminDots = async function() {
    var dotsEl = document.getElementById('officeAdminDots');
    var legendEl = document.getElementById('officeRoomLegend');
    if(!dotsEl) return;
    try {
        var dr = await _sb().from('office_desks').select('nr,room,label,pct_x,pct_y,active').order('nr');
        var rr = await _sb().from('office_rooms').select('id,name,sortierung').order('sortierung');
        var desks = dr.data || [];
        var rooms = rr.data || [];
        
        var roomColorMap = {};
        rooms.forEach(function(r, i) { roomColorMap[r.name] = _officeRoomColors[i % _officeRoomColors.length]; });
        
        var html = '';
        desks.forEach(function(d) {
            if(!d.pct_x || !d.pct_y) return;
            var col = roomColorMap[d.room] || '#999';
            var opacity = d.active !== false ? '1' : '0.35';
            var cursor = _officeAdminDragMode ? 'grab' : 'default';
            var evts = _officeAdminDragMode ? 'onmousedown="officeStartDrag(event,' + d.nr + ')" ontouchstart="officeStartDrag(event,' + d.nr + ')"' : '';
            html += '<div ' + evts + ' data-desk-nr="' + d.nr + '" style="position:absolute;left:' + d.pct_x + '%;top:' + d.pct_y + '%;transform:translate(-50%,-50%);width:26px;height:26px;border-radius:50%;background:' + col + ';border:2px solid #fff;box-shadow:0 1px 4px rgba(0,0,0,0.3);display:flex;align-items:center;justify-content:center;font-size:9px;font-weight:800;color:#fff;pointer-events:auto;cursor:' + cursor + ';opacity:' + opacity + ';z-index:10;user-select:none" title="#' + d.nr + ' ' + (d.label||'') + ' · ' + (d.room||'') + '">' + d.nr + '</div>';
        });
        ...
        if(legendEl) {
            ...
            rooms.forEach(function(r, i) {
                var col = _officeRoomColors[i % _officeRoomColors.length];
                var count = desks.filter(function(d) { return d.room === r.name; }).length;
                lhtml += '<span class="flex items-center gap-1.5 px-2 py-1 rounded-lg text-xs font-semibold" style="background:' + col + '15;color:' + col + ';border:1px solid ' + col + '30"><span style="width:10px;height:10px;border-radius:50%;background:' + col + ';display:inline-block"></span>' + r.name + ' (' + count + ')</span>';
            });
            legendEl.innerHTML = lhtml;
        }
    } catch(e) { console.error('[AdminDots]', e); }
};

// Drag & Drop desk positioning
window.officeStartDrag = function(e, nr) {
    e.preventDefault();
    _officeDraggingDesk = nr;
    var container = document.getElementById('officeFloorplanContainer');
    if(!container) return;
    
    var dot = container.querySelector('[data-desk-nr="' + nr + '"]');
    if(dot) { dot.style.cursor = 'grabbing'; dot.style.zIndex = '100'; dot.style.boxShadow = '0 0 0 3px #EF7D00, 0 2px 8px rgba(0,0,0,0.4)'; }
    
    function onMove(ev) {
        if(!_officeDraggingDesk) return;
    ...
    async function onEnd(ev) {
        ...
        // Get final position
        if(dot) {
            var pctX = parseFloat(dot.style.left);
            var pctY = parseFloat(dot.style.top);
            try {
                await _sb().from('office_desks').update({pct_x: Math.round(pctX*10)/10, pct_y: Math.round(pctY*10)/10}).eq('nr', _officeDraggingDesk);
                _showToast('Platz #' + _officeDraggingDesk + ' verschoben', 'success');
            } catch(err) { _showToast('Fehler: ' + err.message, 'error'); }
            dot.style.cursor = 'grab'; dot.style.zIndex = '10'; dot.style.boxShadow = '0 1px 4px rgba(0,0,0,0.3)';
        }
        _officeDraggingDesk = null;
    }
...
export async function renderOfficeRoomsAdmin() {
    ...
    if(!el) return;
    el.innerHTML = '<div class="text-center py-8 text-gray-400"><div class="animate-spin inline-block w-6 h-6 border-2 border-gray-300 border-t-orange-500 rounded-full"></div><div class="text-xs mt-2">Lade Räume...</div></div>';
    
    try {
        var rr = await _sb().from('office_rooms').select('*').order('sortierung');
        var rooms = rr.data || [];
        // Also load desk counts per room
        var dr = await _sb().from('office_desks').select('nr,room,room_id,label,active');
        var desks = dr.data || [];
        ...
        html += '<div class="text-sm font-bold text-gray-700">' + rooms.length + ' Räume konfiguriert</div>';
        html += '<button onclick="officeRoomAdd()" class="px-4 py-2 rounded-lg text-sm font-semibold text-white bg-vit-orange hover:bg-orange-600 transition">+ Raum hinzufügen</button>';
        html += '</div>';
        
        if(rooms.length === 0) {
            html += '<div class="vit-card p-8 text-center text-gray-400">Keine Räume angelegt</div>';
        } else {
            html += '<div class="space-y-3">';
            rooms.forEach(function(room, idx) {
                var roomDesks = desks.filter(function(d) { return d.room_id === room.id || d.room === room.name; });
                var activeDesks = roomDesks.filter(function(d) { return d.active !== false; });
                var isEditing = window._editingRoomId === room.id;
                
                html += '<div class="vit-card p-4 ' + (room.active === false ? 'opacity-50' : '') + '" style="border-left:4px solid ' + (room.active === false ? '#ccc' : '#EF7D00') + '">';
                
                if(isEditing) {
                    // Edit mode
                    html += '<div class="space-y-3">';
                    html += '<div class="grid grid-cols-2 gap-3">';
                    html += '<div><label class="block text-xs font-semibold text-gray-500 mb-1">Raumname</label>';
                    html += '<input type="text" id="roomEditName_' + room.id + '" value="' + (room.name || '').replace(/"/g, '&quot;') + '" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder="z.B. Büro Nord-1"></div>';
                    html += '<div><label class="block text-xs font-semibold text-gray-500 mb-1">Sortierung</label>';
                    html += '<input type="number" id="roomEditSort_' + room.id + '" value="' + (room.sortierung || 0) + '" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" min="0" max="99"></div>';
                    html += '</div>';
                    html += '<div><label class="block text-xs font-semibold text-gray-500 mb-1">Beschreibung (optional)</label>';
                    html += '<input type="text" id="roomEditDesc_' + room.id + '" value="' + (room.beschreibung || '').replace(/"/g, '&quot;') + '" class="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" placeholder="z.B. 4 Arbeitsplätze, Fensterseite Nord"></div>';
                    html += '<div class="flex gap-2 mt-2">';
                    html += '<button onclick="officeRoomSave(\'' + room.id + '\')" class="px-4 py-2 rounded-lg text-sm font-semibold text-white bg-green-600 hover:bg-green-700">✓ Speichern</button>';
                    html += '<button onclick="officeRoomCancelEdit()" class="px-4 py-2 rounded-lg text-sm font-semibold text-gray-600 bg-gray-100 hover:bg-gray-200">Abbrechen</button>';
                    html += '</div></div>';
                } else {
                    // Display mode
                    html += '<div class="flex items-center justify-between">';
                    html += '<div class="flex items-center gap-3 flex-1">';
                    html += '<div class="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-extrabold" style="background:#FFF4E6;color:#EF7D00">' + (room.sortierung || idx + 1) + '</div>';
                    html += '<div class="flex-1">';
                    html += '<div class="font-bold text-gray-800">' + (room.name || 'Unbenannt') + '</div>';
                    if(room.beschreibung) html += '<div class="text-xs text-gray-500 mt-0.5">' + room.beschreibung + '</div>';
                    html += '<div class="text-xs text-gray-400 mt-0.5">' + activeDesks.length + ' Arbeitsplätze';
                    if(roomDesks.length > activeDesks.length) html += ' (' + (roomDesks.length - activeDesks.length) + ' inaktiv)';
                    html += '</div>';
                    html += '</div></div>';
                    
                    // Action buttons
                    html += '<div class="flex items-center gap-2">';
                    if(idx > 0) html += '<button onclick="officeRoomMove(\'' + room.id + '\', -1)" class="w-7 h-7 rounded flex items-center justify-center text-xs" style="background:var(--c-bg3,#f3f4f6);color:#999" title="Nach oben">↑</button>';
                    if(idx < rooms.length - 1) html += '<button onclick="officeRoomMove(\'' + room.id + '\', 1)" class="w-7 h-7 rounded flex items-center justify-center text-xs" style="background:var(--c-bg3,#f3f4f6);color:#999" title="Nach unten">↓</button>';
                    html += '<button onclick="officeRoomEdit(\'' + room.id + '\')" class="w-7 h-7 rounded flex items-center justify-center text-xs" style="background:#eff6ff;color:#3b82f6" title="Bearbeiten">✏️</button>';
                    html += '<button onclick="officeRoomToggle(\'' + room.id + '\', ' + (room.active !== false ? 'false' : 'true') + ')" class="w-7 h-7 rounded flex items-center justify-center text-xs" style="background:' + (room.active !== false ? '#fef2f2' : '#f0fdf4') + ';color:' + (room.active !== false ? '#ef4444' : '#22c55e') + '" title="' + (room.active !== false ? 'Deaktivieren' : 'Aktivieren') + '">' + (room.active !== false ? '⏸' : '▶') + '</button>';
                    if(roomDesks.length === 0) html += '<button onclick="officeRoomDelete(\'' + room.id + '\', \'' + (room.name || '').replace(/'/g, "\\'") + '\')" class="w-7 h-7 rounded flex items-center justify-center text-xs" style="background:#fef2f2;color:#ef4444" title="Löschen">🗑</button>';
                    html += '</div>';
                    html += '</div>';
                    
                    // Show desks in this room
                    if(roomDesks.length > 0) {
                        html += '<div class="mt-3 pt-3 border-t border-gray-100">';
                        html += '<div class="flex items-center justify-between mb-2">';
                        html += '<div class="text-[10px] font-bold" style="color:var(--c-muted)">'+roomDesks.length+' Arbeitsplätze</div>';
                        html += '<button onclick="officeRoomAddDesk(\'' + room.id + '\', \'' + (room.name || '').replace(/'/g, "\\'") + '\')" class="text-[10px] font-bold px-2 py-0.5 rounded" style="color:#EF7D00;border:1px solid #EF7D00">+ Platz</button>';
                        html += '</div>';
                        html += '<div class="space-y-1">';
                        roomDesks.forEach(function(d) {
                            var opacity = d.active !== false ? '1' : '0.4';
                            html += '<div class="flex items-center justify-between py-1 px-2 rounded" style="background:#f9fafb;opacity:' + opacity + '">';
                            html += '<div class="flex items-center gap-2">';
                            html += '<span class="text-[10px] font-extrabold" style="color:#EF7D00">#' + d.nr + '</span>';
                            html += '<span class="text-[11px] font-semibold" style="color:var(--c-text,#333)">' + (d.label || 'Platz ' + d.nr) + '</span>';
                            html += '</div>';
                        ...
                        html += '<div class="mt-3 pt-3 border-t border-gray-100 flex items-center justify-between">';
                        html += '<div class="text-[10px]" style="color:var(--c-dim)">Keine Plätze</div>';
                        html += '<button onclick="officeRoomAddDesk(\'' + room.id + '\', \'' + (room.name || '').replace(/'/g, "\\'") + '\')" class="text-[10px] font-bold px-2 py-0.5 rounded" style="color:#EF7D00;border:1px solid #EF7D00">+ Platz hinzufügen</button>';
                        html += '</div>';
                    }
                }
        ...
        // Desks without room assignment
        var orphanDesks = desks.filter(function(d) { return !d.room_id && !d.room; });
        if(orphanDesks.length > 0) {
            html += '<div class="mt-4 vit-card p-4 border-l-4 border-yellow-500">';
            html += '<div class="text-sm font-bold text-yellow-700 mb-2">⚠️ ' + orphanDesks.length + ' Arbeitsplätze ohne Raumzuordnung</div>';
            html += '<div class="flex flex-wrap gap-1">';
            orphanDesks.forEach(function(d) {
                html += '<span class="px-2 py-0.5 rounded text-[10px] font-semibold" style="background:#fef9c3;color:#854d0e">#' + d.nr + '</span>';
            });
            html += '</div></div>';
        }
        ...
        el.innerHTML = '<div class="vit-card p-4 text-red-600">Fehler: ' + e.message + '</div>';
    }
}

window._editingRoomId = null;

window.officeRoomEdit = function(id) {
    window._editingRoomId = id;
    renderOfficeRoomsAdmin();
};
window.officeRoomCancelEdit = function() {
    window._editingRoomId = null;
    renderOfficeRoomsAdmin();
};

window.officeRoomSave = async function(id) {
    var name = (document.getElementById('roomEditName_' + id) || {}).value || '';
    var sort = parseInt((document.getElementById('roomEditSort_' + id) || {}).value) || 0;
    var desc = (document.getElementById('roomEditDesc_' + id) || {}).value || '';
    if(!name.trim()) { _showToast('Raumname darf nicht leer sein', 'error'); return; }
    try {
        var oldRoom = await _sb().from('office_rooms').select('name').eq('id', id).single();
        var oldName = oldRoom.data ? oldRoom.data.name : '';
        var r = await _sb().from('office_rooms').update({ name: name.trim(), sortierung: sort, beschreibung: desc.trim() || null }).eq('id', id);
        if(r.error) throw r.error;
        // Also update the room TEXT field on desks that had the old name
        if(oldName && oldName !== name.trim()) {
            await _sb().from('office_desks').update({ room: name.trim() }).eq('room', oldName);
        }
        window._editingRoomId = null;
        _showToast('Raum "' + name.trim() + '" gespeichert', 'success');
        renderOfficeRoomsAdmin();
    } catch(e) { _showToast('Fehler: ' + e.message, 'error'); }
};

window.officeRoomAdd = async function() {
    try {
        var maxSort = 0;
        var rr = await _sb().from('office_rooms').select('sortierung').order('sortierung', {ascending: false}).limit(1);
        if(rr.data && rr.data.length) maxSort = rr.data[0].sortierung || 0;
        var r = await _sb().from('office_rooms').insert([{ name: 'Neuer Raum', sortierung: maxSort + 1 }]).select();
        if(r.error) throw r.error;
        if(r.data && r.data.length) window._editingRoomId = r.data[0].id;
        _showToast('Raum angelegt – jetzt benennen', 'success');
        renderOfficeRoomsAdmin();
    } catch(e) { _showToast('Fehler: ' + e.message, 'error'); console.error('[OfficeRoomAdd]', e); }
};

window.officeRoomToggle = async function(id, active) {
    try {
        var r = await _sb().from('office_rooms').update({ active: active }).eq('id', id);
        if(r.error) throw r.error;
        _showToast(active ? 'Raum aktiviert' : 'Raum deaktiviert', 'success');
        renderOfficeRoomsAdmin();
    } catch(e) { _showToast('Fehler: ' + e.message, 'error'); }
};

window.officeRoomDelete = async function(id, name) {
    if(!confirm('Raum "' + name + '" wirklich löschen?')) return;
    try {
        var r = await _sb().from('office_rooms').delete().eq('id', id);
        if(r.error) throw r.error;
        _showToast('Raum "' + name + '" gelöscht', 'success');
        renderOfficeRoomsAdmin();
    } catch(e) { _showToast('Fehler: ' + e.message, 'error'); }
};

window.officeRoomMove = async function(id, direction) {
    try {
        var rr = await _sb().from('office_rooms').select('id,sortierung').order('sortierung');
        var rooms = rr.data || [];
        var idx = rooms.findIndex(function(r) { return r.id === id; });
        if(idx < 0) return;
        var swapIdx = idx + direction;
        if(swapIdx < 0 || swapIdx >= rooms.length) return;
        // Swap sortierung values
        var s1 = rooms[idx].sortierung;
        var s2 = rooms[swapIdx].sortierung;
        if(s1 === s2) s2 = s1 + direction; // Ensure different
        await _sb().from('office_rooms').update({ sortierung: s2 }).eq('id', rooms[idx].id);
        await _sb().from('office_rooms').update({ sortierung: s1 }).eq('id', rooms[swapIdx].id);
        renderOfficeRoomsAdmin();
    } catch(e) { _showToast('Fehler: ' + e.message, 'error'); }
};

// ═══ DESK MANAGEMENT ═══
window.officeRoomAddDesk = async function(roomId, roomName) {
    try {
        // Find next free desk number
        var rr = await _sb().from('office_desks').select('nr').order('nr', {ascending: false}).limit(1);
        var nextNr = (rr.data && rr.data.length > 0) ? rr.data[0].nr + 1 : 1;
        var label = prompt('Name für neuen Platz #' + nextNr + ' in "' + roomName + '":', roomName + ' - Platz ' + nextNr);
        if(!label) return;
        var r = await _sb().from('office_desks').insert({
            nr: nextNr, room: roomName, room_id: roomId, label: label,
            x: 0, y: 0, pct_x: 50, pct_y: 50, active: true
        });
        if(r.error) throw r.error;
        _showToast('Platz #' + nextNr + ' angelegt', 'success');
        renderOfficeRoomsAdmin();
    } catch(e) { _showToast('Fehler: ' + e.message, 'error'); }
};

window.officeEditDesk = async function(nr) {
    try {
        var rr = await _sb().from('office_desks').select('*').eq('nr', nr).single();
        if(rr.error) throw rr.error;
        var d = rr.data;
        var newLabel = prompt('Platz #' + nr + ' umbenennen:', d.label || '');
        if(newLabel === null) return;
        var r = await _sb().from('office_desks').update({ label: newLabel }).eq('nr', nr);
        if(r.error) throw r.error;
        _showToast('Platz #' + nr + ' umbenannt', 'success');
        renderOfficeRoomsAdmin();
    } catch(e) { _showToast('Fehler: ' + e.message, 'error'); }
};

window.officeToggleDesk = async function(nr, active) {
    try {
        var r = await _sb().from('office_desks').update({ active: active }).eq('nr', nr);
        if(r.error) throw r.error;
        _showToast('Platz #' + nr + (active ? ' aktiviert' : ' deaktiviert'), 'success');
        renderOfficeRoomsAdmin();
    } catch(e) { _showToast('Fehler: ' + e.message, 'error'); }
};
...
// Strangler Fig
const _exports = {approveUser,switchApproveEbene,closeApproveModal,confirmApprove,rejectUser,openRollenModal,closeRollenModal,saveRollen,getStandortName,getPartnerMitarbeiter,filterPartnerMa,showMaTab,renderPartnerMitarbeiter,openEmployeeToolsModal,closeEmpToolsModal,saveEmployeeTools,openEditEmployeeModal,closeEditEmpModal,saveEditEmployee,renderMaToolsMatrix,renderMaKosten,deactivateMa,openNeuerMaModal,switchNewMaEbene,closeNeuerMaModal,switchNewMaEmailType,updateNewMaEmail,saveNeuerMa,openEditMaModal,switchEditEbene,closeEditMaModal,saveEditMa,loginAs,deleteMa,openNeuerStandortModal,closeNeuerStdModal,saveNeuerStandort,showSettingsTab,renderModulStatusList,renderHqModulStatusList,_renderModulRow,toggleModuleExpand,onMove,onEnd,renderOfficeRoomsAdmin,renderDemoModulList,setDemoModulStatus,setDemoTabStatus,setDemoWidgetStatus,setAllDemoStatus,setTabStatus,setWidgetStatus,setModulStatus,setHqModulStatus,setHqTabStatus,setHqWidgetStatus,renderHqEinstellungen,togglePermission,renderHqRechteMatrixBody,loadHqRechteMatrix,toggleHqPermission};
Object.entries(_exports).forEach(([k, fn]) => { window[k] = fn; });
console.log('[user-management.js] Module loaded - ' + Object.keys(_exports).length + ' exports registered');

// === WINDOW REGISTRATION ===
// Auto-register 59 exports on window for onclick compatibility
window.approveUser = approveUser;
window.switchApproveEbene = switchApproveEbene;
window.closeApproveModal = closeApproveModal;
window.confirmApprove = confirmApprove;
window.rejectUser = rejectUser;
window.openRollenModal = openRollenModal;
window.closeRollenModal = closeRollenModal;
window.saveRollen = saveRollen;
window.getStandortName = getStandortName;
window.getPartnerMitarbeiter = getPartnerMitarbeiter;
window.filterPartnerMa = filterPartnerMa;
window.showMaTab = showMaTab;
window.renderPartnerMitarbeiter = renderPartnerMitarbeiter;
window.openEmployeeToolsModal = openEmployeeToolsModal;
window.closeEmpToolsModal = closeEmpToolsModal;
window.saveEmployeeTools = saveEmployeeTools;
window.openEditEmployeeModal = openEditEmployeeModal;
window.closeEditEmpModal = closeEditEmpModal;
window.saveEditEmployee = saveEditEmployee;
window.renderMaToolsMatrix = renderMaToolsMatrix;
window.renderMaKosten = renderMaKosten;
window.deactivateMa = deactivateMa;
window.openNeuerMaModal = openNeuerMaModal;
window.switchNewMaEbene = switchNewMaEbene;
window.closeNeuerMaModal = closeNeuerMaModal;
window.switchNewMaEmailType = switchNewMaEmailType;
window.updateNewMaEmail = updateNewMaEmail;
window.saveNeuerMa = saveNeuerMa;
window.openEditMaModal = openEditMaModal;
window.switchEditEbene = switchEditEbene;
window.closeEditMaModal = closeEditMaModal;
window.saveEditMa = saveEditMa;
window.loginAs = loginAs;
window.deleteMa = deleteMa;
window.openNeuerStandortModal = openNeuerStandortModal;
window.closeNeuerStdModal = closeNeuerStdModal;
window.saveNeuerStandort = saveNeuerStandort;
window.showSettingsTab = showSettingsTab;
window.renderModulStatusList = renderModulStatusList;
window.renderHqModulStatusList = renderHqModulStatusList;
window._renderModulRow = _renderModulRow;
window.toggleModuleExpand = toggleModuleExpand;
window.renderOfficeRoomsAdmin = renderOfficeRoomsAdmin;
window.renderDemoModulList = renderDemoModulList;
window.setDemoModulStatus = setDemoModulStatus;
window.setDemoTabStatus = setDemoTabStatus;
window.setDemoWidgetStatus = setDemoWidgetStatus;
window.setAllDemoStatus = setAllDemoStatus;
window.setTabStatus = setTabStatus;
window.setWidgetStatus = setWidgetStatus;
window.setModulStatus = setModulStatus;
window.setHqModulStatus = setHqModulStatus;
window.setHqTabStatus = setHqTabStatus;
window.setHqWidgetStatus = setHqWidgetStatus;
window.renderHqEinstellungen = renderHqEinstellungen;
window.togglePermission = togglePermission;
window.renderHqRechteMatrixBody = renderHqRechteMatrixBody;
window.loadHqRechteMatrix = loadHqRechteMatrix;
window.toggleHqPermission = toggleHqPermission;
// === END REGISTRATION ===
