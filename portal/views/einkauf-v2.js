...
// Theme: portal-kompatibel (Light/Dark via body.dark)
// ============================================================

const EINKAUF_V2_VERSION = 4;
...
function statusBadge(status, map) {
  ...
  return `<span class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs border ${cls}">${cfg.icon} ${esc(label)}</span>`;
}

// Input/Select Klassen für Modal-Formulare
    ...
    <div class="p-6 max-w-7xl mx-auto">
      <div class="flex items-center justify-between mb-6">
        <div>
          ...
          <p class="text-gray-500 text-sm">${isHQ ? 'HQ-Cockpit – alle Lieferanten & Verhandlungsstatus' : 'Eure Lieferanten mit vit:bikes Konditionen'}</p>
        </div>
        ${isHQ ? `
    ...
    <div id="einkauf-modal" class="fixed inset-0 bg-black/50 z-50 hidden flex items-center justify-center p-4">
      <div class="bg-white rounded-xl border border-gray-200 shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div class="flex items-center justify-between p-6 border-b border-gray-200">
          <h2 id="einkauf-modal-title" class="text-lg font-semibold text-gray-800">Lieferant bearbeiten</h2>
...
function einkaufV2RenderStats(data) {
  ...
  statsEl.innerHTML = `
    ${statCard('✅', 'Fixiert', fixiert, 'text-green-600')}
    ${statCard('🟡', 'In Verhandlung', verhandlung, 'text-orange-500')}
    ${statCard('⭐', 'Kernsortiment', kern, 'text-blue-600')}
    ${statCard('❌', 'Gescheitert / ausgelaufen', gescheitert, 'text-red-500')}
  `;
}
    ...
    if (isHQ) {
      ...
      liste.forEach(l => {
        const kernBadge = l.kernsortiment
          ...
          : l.zusatz
            ? `<span class="text-xs bg-gray-100 text-gray-500 border border-gray-200 px-1.5 py-0.5 rounded">➕ Zusatz</span>`
            : '';

        const kontakt = l.email_innendienst
          ? `<a href="mailto:${esc(l.email_innendienst)}" class="text-orange-500 hover:text-orange-600 text-xs">${esc(l.email_innendienst)}</a>`
        ...
        html += `<tr class="border-b border-gray-100 hover:bg-orange-50/30 cursor-pointer transition-colors" onclick="window.einkaufV2OpenEdit('${l.id}')">
          <td class="px-4 py-3">
            <div class="font-medium text-gray-800">${esc(l.lieferant)}</div>
          ...
          <td class="px-4 py-3 text-gray-500 text-xs">${esc(l.konditionen_detail || '–')}</td>
          <td class="px-4 py-3">${kontakt}</td>
          <td class="px-4 py-3">
            <button onclick="event.stopPropagation(); window.einkaufV2OpenEdit('${l.id}')"
              class="text-gray-300 hover:text-orange-500 transition-colors p-1">
      ...
      liste.forEach(l => {
        const b2bBtn = l.b2b_shop
          ? `<span class="text-xs text-blue-500">🔗 ${esc(l.b2b_shop)}</span>`
          : '';
        const emailBtn = l.email_innendienst
          ? `<a href="mailto:${esc(l.email_innendienst)}" onclick="event.stopPropagation()"
               class="text-xs text-orange-500 hover:text-orange-600 truncate block">✉ ${esc(l.email_innendienst)}</a>`
          : '';
        const telefonBtn = l.telefon_innendienst
          ? `<a href="tel:${esc(l.telefon_innendienst)}" onclick="event.stopPropagation()"
               class="text-xs text-gray-500 hover:text-gray-700 block">📞 ${esc(l.telefon_innendienst)}</a>`
          : '';

        html += `
          <div class="vit-card border rounded-xl p-4 hover:border-orange-300 transition-colors">
            <div class="flex items-start justify-between gap-2 mb-3">
              <div>
                <div class="font-semibold text-gray-800">${esc(l.lieferant)}</div>
            ...
            ${l.konditionen_detail ? `<div class="text-xs text-gray-500 mb-2">📋 ${esc(l.konditionen_detail)}</div>` : ''}
            <div class="space-y-1 border-t border-gray-100 pt-2 mt-2">
              ${emailBtn}${telefonBtn}${b2bBtn}
            </div>
          </div>`;
      });
      html += `</div>`;
...
window.einkaufV2OpenEdit = async function(id) {
  ...
  const lieferant = id ? (_einkaufAllData.find(l => l.id === id) || null) : null;
  title.textContent = lieferant ? `Bearbeiten: ${lieferant.lieferant}` : 'Neuer Lieferant';
  ...
  const statusOpts = Object.keys(STATUS_CONFIG).map(s =>
    `<option value="${esc(s)}" ${val('status_konditionen') === s ? 'selected' : ''}>${s}</option>`).join('');
  const ihtOpts = Object.keys(IHT_CONFIG).map(s =>
    `<option value="${esc(s)}" ${val('status_iht') === s ? 'selected' : ''}>${s}</option>`).join('');
    ...
    <form id="einkauf-form" class="space-y-4" onsubmit="event.preventDefault(); window.einkaufV2Save('${id || ''}')">
      <div class="grid grid-cols-2 gap-4">
        <div class="col-span-2">
          ...
          <input name="lieferant" required value="${esc(val('lieferant'))}" class="${INP}">
        </div>
        <div>
      ...
      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="${LBL}">Status Konditionen</label>
          <select name="status_konditionen" class="${SEL}">${statusOpts}</select>
        </div>
        <div>
          <label class="${LBL}">Status IHT</label>
          <select name="status_iht" class="${SEL}">${ihtOpts}</select>
        </div>
      </div>

      <div>
        <label class="${LBL}">Konditionen-Bezeichnung</label>
        <input name="konditionen_detail" value="${esc(val('konditionen_detail'))}" class="${INP}" placeholder="z.B. Konditionen Simplon">
      </div>

      <div class="grid grid-cols-2 gap-4">
        <div>
          <label class="${LBL}">E-Mail Innendienst</label>
          <input name="email_innendienst" type="email" value="${esc(val('email_innendienst'))}" class="${INP}">
        </div>
        <div>
          <label class="${LBL}">Telefon Innendienst</label>
          <input name="telefon_innendienst" value="${esc(val('telefon_innendienst'))}" class="${INP}">
        </div>
        <div>
          <label class="${LBL}">B2B-Shop Name</label>
          <input name="b2b_shop" value="${esc(val('b2b_shop'))}" class="${INP}">
        </div>
        <div>
          <label class="${LBL}">E-Mail Registrierung</label>
          <input name="email_registrierung" type="email" value="${esc(val('email_registrierung'))}" class="${INP}">
        </div>
      </div>

      <div>
        <label class="${LBL}">Notizen (intern)</label>
        <textarea name="notizen" rows="2" class="${INP} resize-none"
          placeholder="Interne Notizen…">${esc(val('notizen'))}</textarea>
      </div>
      ...
      <div class="flex items-center gap-3 pt-2 border-t border-gray-200">
        ...
        ${id ? `<button type="button" onclick="window.einkaufV2Delete('${id}')"
          class="px-3 py-2 bg-red-50 hover:bg-red-100 text-red-600 rounded-lg text-sm transition-colors border border-red-200">
          Löschen
        </button>` : ''}
      </div>
    </form>`;
