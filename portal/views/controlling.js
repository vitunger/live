        ...
        if(detailRows) {
            ...
            if(Math.abs(detErgebnis - (parseFloat(b.ergebnis_vor_steuern)||0)) > 1) {
                ...
                _sb().from('bwa_daten').update(fixData).eq('id', bwaId).then(function(r){
                    if(!r.error) { console.debug('[BWA] DB values corrected for', bwaId); loadBwaList(); }
