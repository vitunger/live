...
// Migrated from inline/bwa-cockpit.js to ES Module
...
export { updateBwaDeadlineWidget, renderHqBwaStatus, showKpiReport as _bwaCockpitShowKpiReport };
...
async function showKpiReport(bwaMo, rating) {
    ...
    try {
        ...
        grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;color:#9ca3af;font-size:12px;padding:8px">⚠️ ' + _escH(err.message) + '</div>';
...
async function renderHqBwaStatus() {
    ...
    try {
        ...
        if(tbody) tbody.innerHTML = '<tr><td colspan="7" class="py-6 text-center text-red-400 text-sm">⚠️ Fehler beim Laden: ' + _escH(err.message) + '</td></tr>';
    }
}
