...
/* ── Tiny Components ─────────────────────────────── */
export function Particle({x,y,emoji,delay}){return <div style={{position:"fixed",left:x,top:y,fontSize:"2rem",pointerEvents:"none",zIndex:9999,animation:`fly 1.2s ease-out ${delay}s forwards`,opacity:0}}>{emoji}</div>}
export function Heat({heat}){return <div style={{display:"flex",gap:2,alignItems:"center"}}>{[1,2,3,4,5].map(i=><div key={i} style={{width:7,height:7,borderRadius:"50%",background:i<=heat?(heat>=4?"#FF6B35":heat>=3?"#F7C948":"#CBD5E0"):"#E2E8F0",transition:"all .3s"}}/>)}<span style={{fontSize:9,marginLeft:3,color:"#999"}}>{heat>=4?"🔥":heat>=3?"☀️":"❄️"}</span></div>}

export function TodoBadge({todos}){
...
export function AgingBadge({deal}){
