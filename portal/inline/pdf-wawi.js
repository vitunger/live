// vit:bikes Partner Portal — PDF Parser + WaWi Integration
...
(function(){
    ...
    window.wawiSaveConnection = async function() {
        ...
        try {
            ...
            _wawiConnection = r.data;
            showToast('WaWi-Verbindung gespeichert ✅', 'success');
            wawiRenderStatus(r.data);
            document.getElementById('wawiSyncPanel').style.display = 'block';
            loadWawiSyncLog();
        } catch(e) {
    ...
    window.wawiTriggerSync = async function(module) {
        ...
        try {
            ...
            var data = r.data;
            if(data && data.success) {
                showToast('✅ ' + module + ': ' + (data.total || 0) + ' Datensätze in ' + (data.dauer_ms || 0) + 'ms', 'success');
                loadWawiSyncLog();
                loadWawiDataPreview();
            } else {
                showToast('❌ ' + (data && data.error ? data.error : 'Fehler'), 'error');
            }
    ...
    function renderParseCard(fileName, parsed, error) {
        ...
        var leasing = parsed.meta.ist_leasing ? '<span class="text-[10px] px-1.5 py-0.5 rounded-full bg-purple-100 text-purple-700 font-semibold">🚲 '+escH(parsed.meta.leasing_anbieter||'Leasing')+'</span>' : '';
        return '<div class="flex items-start gap-3"><div class="w-8 h-8 bg-green-100 rounded-full flex items-center justify-center text-green-600 flex-shrink-0 mt-0.5">✓</div>'
            ...
            +'<span>'+escH(parsed.customer.name||'–')+'</span>'
    ...
    window.loadWawiBelege = async function() {
        ...
        data.forEach(function(b) {
            ...
            h += '<td class="px-4 py-3 text-gray-800 font-medium truncate max-w-[200px]">'+escH(b.kunde_name||'–')+'</td>';
            h += '<td class="px-4 py-3 text-gray-600">'+escH(b.verkaeufer||'–')+'</td>';
            ...
            h += '<td class="px-4 py-3">'+(b.ist_leasing?'<span class="text-[10px] px-1.5 py-0.5 rounded-full bg-purple-100 text-purple-700 font-semibold">'+escH(b.leasing_anbieter||'Ja')+'</span>':'<span class="text-xs text-gray-300">–</span>')+'</td>';
            h += '</tr>';
        });
        h += '</tbody></table></div>';
    ...
    window.showWawiBelegDetail = async function(id) {
        ...
        h += '<div><span class="text-gray-400 text-xs block">Kunde</span><strong>'+escH(b.kunde_name||'–')+'</strong>'+(b.kunde_ort?'<br><span class="text-xs text-gray-500">'+escH(b.kunde_plz||'')+' '+escH(b.kunde_ort)+'</span>':'')+'</div>';
        h += '<div><span class="text-gray-400 text-xs block">Verkäufer</span><strong>'+escH(b.verkaeufer||'–')+'</strong></div>';
        h += '<div><span class="text-gray-400 text-xs block">Zahlung</span><strong>'+escH(b.zahlungsbedingung||'–')+'</strong></div>';
        if(b.ist_leasing) h += '<div class="col-span-2"><span class="text-[10px] px-2 py-0.5 rounded-full bg-purple-100 text-purple-700 font-bold">🚲 Leasing: '+escH(b.leasing_anbieter||'Ja')+'</span></div>';
        h += '</div>';
        ...
        pos.forEach(function(p){
            ...
            h += '<td class="py-1.5 pr-2 font-mono text-xs text-gray-500">'+escH(p.art_nr||'–')+'</td>';
