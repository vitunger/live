// vit:bikes Partner Portal — Video Pipeline Helpers
// Extracted from index.html lines 13183-13984
// ============================================================
(function(){
    'use strict';
    ...
    function vpDate(d) { if(!d) return '–'; return new Date(d).toLocaleDateString('de-DE'); }
    function vpDateTime(d) { if(!d) return '–'; return new Date(d).toLocaleString('de-DE',{day:'2-digit',month:'2-digit',year:'2-digit',hour:'2-digit',minute:'2-digit'}); }
    function vpFileSize(bytes) { if(!bytes) return '–'; if(bytes<1024*1024) return (bytes/1024).toFixed(0)+' KB'; return (bytes/(1024*1024)).toFixed(1)+' MB'; }
